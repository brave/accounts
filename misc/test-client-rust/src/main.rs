        ...
        if keys_array.is_empty() {
            ...
            for (i, key) in keys_array.iter().enumerate() {
                ...
                if args.verbose {
                    ...
                    if let Some(created) = key.get("createdAt").and_then(|v| v.as_str()) {
...
fn store_key(args: &CliArgs) {
    ...
    let mut body: HashMap<&str, Value> = HashMap::new();
    body.insert("name", key_name.clone().into());
    ...
    if status == reqwest::StatusCode::NO_CONTENT {
        println!("Key '{}' stored successfully", key_name);
    } else {
...
fn get_key(args: &CliArgs) {
    ...
    let (response, status) = make_request(
        args,
        reqwest::Method::GET,
        &format!("/v2/keys/{}", key_name),
        Some(auth_token),
        None,
    );

    if status == reqwest::StatusCode::OK {
        println!("Key '{}' found:", key_name);
        if let Some(material) = response.get("keyMaterial").and_then(|v| v.as_str()) {
            println!("Material: {}", material);
        }
        if args.verbose {
            if let Some(created) = response.get("createdAt").and_then(|v| v.as_str()) {
                println!("Created: {}", created);
            }
